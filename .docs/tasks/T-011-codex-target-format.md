...
- A target format specification document. Location options (decide at task start):
  - `.claude/conventions/target-codex.md` — treats target specs as kit conventions
...
- **Frontmatter mapping:** which fields in the `.claude/` source map to which Codex fields; what to do with fields that have no Codex equivalent.
...
- **Concrete before/after example:** show a snippet of `.claude/skills/<name>/SKILL.md` and its Codex output side by side.
