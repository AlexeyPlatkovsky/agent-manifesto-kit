...
Author the `provider-adapter` agent: translates source assets in `.claude/` to other provider formats (`.codex/`, `.ai/`) per the target specs from [T-011](T-011-codex-target-format.md) and [T-012](T-012-ai-agnostic-target-format.md).
...
Inputs:
- A source asset path under `.claude/` (skill or agent).
...
- `when_not_to_use` covers: source asset already exists in `.claude/` and consumer is on Claude (no translation needed); pipelines (deferred to v0.2+); arbitrary file rewriting outside the adapter contract.
...
- The adapter is asymmetric: source is always `.claude/`, never the other way around. This is intentional — see [architecture.md § Adapter flow](../architecture.md).
