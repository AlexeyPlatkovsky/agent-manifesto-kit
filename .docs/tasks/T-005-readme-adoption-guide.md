...
- **Repository layout:** mini version of [architecture.md § Top-level repository layout](../architecture.md), with the two-folder distinction (`.claude/` ships, `.ai/` does not) called out explicitly.
- **How to adopt a capability:**
  - Claude direct path (copy `.claude/<type>/<name>/` into the consumer project).
  - Codex via adapter.
