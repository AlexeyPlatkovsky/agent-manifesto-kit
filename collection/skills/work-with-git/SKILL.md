...
- Perform git operations for a task: inspect state, create branches, stage files, commit, push, and report.

## Prerequisites

Before running git operations, confirm:
- Requires the Git CLI: <https://git-scm.com/docs/git>.
- Git is available; verify with `git --version`. If unavailable, stop and ask the user to install Git or provide another environment.
...
When branch creation is requested, use the task classification already stated by the caller when available. If branch choice depends on task classification and classification is absent, ask for the missing input instead of classifying the task inside this skill.
...
Detect the default remote branch first when a remote is available:
```bash
git fetch origin
...
If no remote or default remote branch is available, ask the user which base ref to use instead of treating local-only branch creation as blocked.

State the outcome before proceeding:
- **Skipped**: `<reason>`
- **Created**: `<branch-name>` from `origin/<default-branch>` or approved local base ref
