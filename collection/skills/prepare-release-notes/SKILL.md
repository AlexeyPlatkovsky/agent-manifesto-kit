...
- Git is installed and available; verify with `git --version` and use <https://git-scm.com/docs> for command semantics.
- The target is a Git repository; verify with `git rev-parse --is-inside-work-tree`.
- The changelog file is identified and readable, or the user has approved the file to create.
- The release range has a clear baseline. Use `HEAD` as the upper bound unless the user provides another commit or ref.
...
- Stop and report blocked when no git history exists; ask for the release range or commit to use as the baseline in `Blockers`.
- Stop and report blocked when no git tags exist and no version heading is present in the changelog; ask for the version number and baseline.
- Stop and report blocked when no changelog file exists (no `CHANGES.md`, `CHANGELOG.md`, or equivalent); ask for the filename and location.
- Stop and report blocked when git tags and changelog version headings disagree about the latest release baseline.
...
2. Inspect commits in range; include only those that modify meaningful product behavior.
...
6. Group changes into `Added`, `Changed`, `Fixed`, `Internal`; omit empty categories.
7. Write the entry per the Entry Format below and place it at the top of the changelog file.
...
- Categories:
  ...
  - `Fixed` — bug fixes
  - `Internal` — release-relevant dependency, refactor, or build changes that affect shipped behavior, compatibility, security posture, or operator-visible release behavior
...
Place the newest entry at the top of the changelog file. Omit empty categories, including `Internal`.
...
Completion requires a non-empty changelog entry for the target version.
