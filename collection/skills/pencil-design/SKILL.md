...
7. Verify changes via `get_screenshot` and `batch_get`.

If the target file, frame, or screen is unclear, ask one concise clarification before editing.
...
Before reporting completion, verify that layout problems are absent or explained, significant edited nodes can be read back with `batch_get`, and a screenshot was captured for visual changes unless unavailable.
