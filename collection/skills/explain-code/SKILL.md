...
- Explain how code actually works by tracing flow, understanding data transformations, or mapping component relationships.
...
6. When several entry points or flows could answer the question, choose the one most directly matching the user's wording and state that scope. If two or more are equally plausible and would produce different explanations, stop and ask for clarification.
...
Use `blocked` when important behavior cannot be verified from implementation evidence. Record narrower verified scope in `Scope` and missing areas in `Unknowns` when only part of the requested behavior can be explained.
