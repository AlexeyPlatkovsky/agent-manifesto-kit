...
Pre-defined routing plan for introducing or expanding a Spec-Driven Development context
`docs/` tree in a project that already has documentation, code, or both. It sequences the
SDD bundle's gap analysis, document authoring, Taskpilot feature tracking, and review so
existing material is reconciled rather than discarded.
...
| 5. Features | `Skill: sdd-feature-author` (once per extracted feature item) | `Skill: sdd-feature-author - output below` |
...
- Taskpilot project `amk` for canonical work and feature records

## Stop Conditions

- The gap analysis reports a fundamental conflict requiring a user decision — resolve at
  stage 3 before authoring.
- A doc-authoring or Taskpilot feature step blocks — resolve before advancing.
