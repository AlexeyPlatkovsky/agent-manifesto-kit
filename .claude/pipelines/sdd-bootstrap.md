...
Pre-defined routing plan for establishing a Spec-Driven Development context `docs/` tree in
a project from scratch. It sequences the SDD bundle's skills, Taskpilot feature records, and
agents so the result conforms to the `sdd-doc-set` convention.
...
| 7. Features | `Skill: sdd-feature-author` (once per Taskpilot feature item) | `Skill: sdd-feature-author - output below` |
...
On the `Lean` tier, skip stages 4 and 5. Stage 10 is opt-in and may be declined. Do not
