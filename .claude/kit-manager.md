...
A pipeline under `.claude/pipelines/<name>.md` is a pre-baked routing plan for a recurring workflow. When a pipeline matches the current request, this manager adopts its stages verbatim and records the pipeline by name in the routing artifact. The manager remains responsible for the routing artifact itself; pipelines do not replace `kit-manager`. Pipelines do not embed implementation logic and do not emit their own output artifact.
...
| Completion gate | expected `Skill: kit-task-complete - output below` artifact |
