...
- The user requests a new skill under `collection/skills/` or `.ai/skills/`.
...
- `.ai/conventions/ambiguity-resolution.md`
...
- **Portability honored.** Skills under `collection/skills/` are project-agnostic per `.ai/conventions/capability-portability.md`. Skills under `.ai/skills/` may reference local authorities by path.
...
The pipeline itself does not emit an artifact. Each stage emits its own contract artifact as listed above. The closure stage (`task-complete`) records the full sequence.
