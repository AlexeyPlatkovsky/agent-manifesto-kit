...
This skill follows `.ai/docs/skill-template.md`. Cross-stage sequencing — structural review, enrichment, acceptance testing, validation, documentation maintenance, and closure — lives in `.ai/pipelines/skill-authoring.md` and is invoked by the manager, not by this skill.

## Scope

- Produce a new `SKILL.md` under `.ai/skills/<name>/` or `collection/skills/<name>/`.
...
- Target location: `.ai/skills/` (workshop) or `collection/skills/` (product).
- Mode: `new` or `revise`.
...
- `.ai/docs/skill-template.md`
- `.ai/conventions/capability-portability.md`
...
3. Draft `SKILL.md` per `.ai/docs/skill-template.md`: YAML frontmatter with `name` and `description`, then Markdown body containing `Scope`, `Procedure`, and `Output Contract`. Add `Prerequisites`, `Inputs`, `Boundaries`, or `Stop Conditions` when relevant.
4. Apply `.ai/conventions/capability-portability.md`:
   ...
   - Skills under `.ai/skills/` may reference local authorities by path.
...
- Target location is ambiguous (`.ai/` vs `collection/`) — stop and request the user choose.
...
- Do not invoke `instruction-evaluator`, `artifact-enricher`, or `artifact-acceptance-tester` from this skill. Those stages are sequenced by `.ai/pipelines/skill-authoring.md`.
- Do not produce agents, conventions, pipelines, or routing artifacts.
- Do not edit `AGENTS.md`, `.ai/kit-manager.md`, or other authority artifacts as part of authoring a single skill.
...
| Mode | new / revise |
| Location | workshop (`.ai/skills/`) / product (`collection/skills/`) |
